...
if __name__ == "__main__":
    ...
    for k in range(0, n+1):
        p1 += C1[k]*Bk_4(k, t)
    plt.plot(t_space, [p1.subs(t, ti) for ti in t_space],'r')

    # at time t = 0.8, we want to concatenate P1 with another Bezier curve P2.
    T = 0.25
    plt.plot(T, p1.subs(t, T), 'bo')
    ...
    for k in range(0, n+1):
        p2 += C2[k]*Bk_4(k, t)
    plt.plot(t_space + T, [p2.subs(t, ti) for ti in t_space],'b')
    ...
    for k in range(0, n+1):
        p3 += C3[k]*Bk_4(k, t)
    plt.plot(t_space + T, [p3.subs(t, ti) for ti in t_space],'b')
