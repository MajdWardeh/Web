...
from quadrotor_msgs.msg import TrajectoryPoint, Trajectory
# import tf
...
class TrajectoryNavigator:
    ...
    def __init__(self, twist_data_length=20):
        ...
        # Publishers:
        self.trajectory_pub = rospy.Publisher('/hummingbird/autopilot/trajectory', Trajectory, queue_size=1)
    ...
    def timerCallback(self, timerMsg):
        if self.ti == -1 or self.ti >= self.max_ti_index:
            return
        ...
        q = [0, 0, 0, 1]
        pose = Pose()
        pose.position.x = self.traj_position[self.ti, 0]
        pose.position.y = self.traj_position[self.ti, 1]
        pose.position.z = self.traj_position[self.ti, 2]
        ...
        vel_twist = Twist()
        vel_twist.linear.x = self.traj_linearVel[self.ti, 0]
        vel_twist.linear.y = self.traj_linearVel[self.ti, 1]
        vel_twist.linear.z = self.traj_linearVel[self.ti, 2]
        vel_twist.angular.x = 0
        vel_twist.angular.y = 0
        vel_twist.angular.z = 0

        Acc_twist = Twist()
        Acc_twist.linear.x = self.traj_linearAcc[self.ti, 0]
        Acc_twist.linear.y = self.traj_linearAcc[self.ti, 1]
        Acc_twist.linear.z = self.traj_linearAcc[self.ti, 2]
        Acc_twist.angular.x = 0
        Acc_twist.angular.y = 0
        Acc_twist.angular.z = 0

        Jerk_twist = Twist()
        Jerk_twist.linear.x = self.traj_linearJerk[self.ti, 0]
        Jerk_twist.linear.y = self.traj_linearJerk[self.ti, 1]
        Jerk_twist.linear.z = self.traj_linearJerk[self.ti, 2]
        Jerk_twist.angular.x = 0
        Jerk_twist.angular.y = 0
        Jerk_twist.angular.z = 0
        
        Snap_twist = Twist()
        Snap_twist.linear.x = self.traj_linearSnap[self.ti, 0]
        Snap_twist.linear.y = self.traj_linearSnap[self.ti, 1]
        Snap_twist.linear.z = self.traj_linearSnap[self.ti, 2]
        Snap_twist.angular.x = 0
        Snap_twist.angular.y = 0
        Snap_twist.angular.z = 0
        

        point = TrajectoryPoint()
        point.pose = pose
        point.velocity = vel_twist
        point.acceleration = Acc_twist
        point.jerk = Jerk_twist
        point.snap = Snap_twist

        point.heading = self.traj_orientation[self.ti]
        point.heading_rate = self.traj_angularVel[self.ti]
        point.heading_acceleration = self.traj_angularAcc[self.ti]
    ...
    def publishTheWholeTrajectory(self, x_star, Px, Py, Pz, Pyaw):
        _, T_star = Px.applyOptVect(x_star)
        num = int((T_star/self.trajectorySamplingPeriod))
        ...
        self.traj_linearJerk = np.zeros((num, 3))
        self.traj_linearJerk[:, 0] = Px.diP_dt(x_star, 3, t_list)
        self.traj_linearJerk[:, 1] = Py.diP_dt(x_star, 3, t_list)
        self.traj_linearJerk[:, 2] = Pz.diP_dt(x_star, 3, t_list)
 
        self.traj_linearSnap = np.zeros((num, 3))
        self.traj_linearSnap[:, 0] = Px.diP_dt(x_star, 4, t_list)
        self.traj_linearSnap[:, 1] = Py.diP_dt(x_star, 4, t_list)
        self.traj_linearSnap[:, 2] = Pz.diP_dt(x_star, 4, t_list)
        ...
        for k, ti in enumerate(np.linspace(0, T_star, num, endpoint=True)):
            point = TrajectoryPoint()
            # Pyaw = self.traj_orientation[ti]
            # q = Rotation.from_euler('z', Pyaw).as_quat()
            q = [0, 0, 0, 1]
            point.pose.position.x = self.traj_position[k, 0]
            point.pose.position.y = self.traj_position[k, 1]
            point.pose.position.z = self.traj_position[k, 2]
            point.pose.orientation.x = q[0] 
            point.pose.orientation.y = q[1] 
            point.pose.orientation.z = q[2] 
            point.pose.orientation.w = q[3] 

            point.velocity.linear.x = self.traj_linearVel[k, 0]
            point.velocity.linear.y = self.traj_linearVel[k, 1]
            point.velocity.linear.z = self.traj_linearVel[k, 2]
            point.velocity.angular.x = 0
            point.velocity.angular.y = 0
            point.velocity.angular.z = 0

            point.acceleration.linear.x = self.traj_linearAcc[k, 0]
            point.acceleration.linear.y = self.traj_linearAcc[k, 1]
            point.acceleration.linear.z = self.traj_linearAcc[k, 2]
            point.acceleration.angular.x = 0
            point.acceleration.angular.y = 0
            point.acceleration.angular.z = 0

            point.jerk.linear.x = self.traj_linearJerk[k, 0]
            point.jerk.linear.y = self.traj_linearJerk[k, 1]
            point.jerk.linear.z = self.traj_linearJerk[k, 2]
            point.jerk.angular.x = 0
            point.jerk.angular.y = 0
            point.jerk.angular.z = 0
            
            point.snap.linear.x = self.traj_linearSnap[k, 0]
            point.snap.linear.y = self.traj_linearSnap[k, 1]
            point.snap.linear.z = self.traj_linearSnap[k, 2]
            point.snap.angular.x = 0
            point.snap.angular.y = 0
            point.snap.angular.z = 0
            
            point.heading = self.traj_orientation[k]
            point.heading_rate = self.traj_angularVel[k]
            point.heading_acceleration = self.traj_angularAcc[k]
    ...
    def run(self, startPose, endPose):
        ...
        self.publishTheWholeTrajectory(x_star, Px, Py, Pz, Pyaw)
