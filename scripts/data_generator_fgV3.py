...
class StateAggregator:
    ...
    def runOneEpoch(self): 
        ...
        while not self.epoch_finished and not rospy.is_shutdown():
            ...
            if not self.lastOdomMsg is None:
                dronePosition = self.lastOdomMsg.pose.pose.position
                if la.norm(np.array([dronePosition.x, dronePosition.y, dronePosition.z]) - self.targetGateCOM) < self.droneGateDistanceLowerBound:
                    print('drone is close to the gate. epoch will finish soon.')
                    droneIsCloseToGate = True

            if droneIsCloseToGate and len(self.stateAggregation_tidList) == 0:
                    self.epoch_finished = True
                    break

            if not droneIsCloseToGate and len(self.stateAggregation_tidList) < 3:
                continue
