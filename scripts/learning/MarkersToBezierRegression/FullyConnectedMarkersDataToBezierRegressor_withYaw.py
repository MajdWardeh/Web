...
class Network:
    ...
    def _createModel(self):
        ...
        model = Model(inputs=inputLayer, outputs=[positionOutput, yawOutput])
        return model
...
class Training:
    ...
    def trainModel(self):
        ...
        try:
            history = self.model.fit(
                x=self.trainGen, epochs=5000, 
                validation_data=self.testGen, validation_steps=5, 
                # callbacks=[tensorboardCallback],
                verbose=1, workers=4, use_multiprocessing=True)
            with open(os.path.join(self.saveHistoryDir, 'weights_MarkersToBeizer_FC_withYaw_{}.pkl'.format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))), 'wb') as file_pi:
    ...
    def testModel(self):
        self.model.load_weights(os.path.join(self.model_weights_dir, 'weights_MarkersToBeizer_FC_scratch_20210809-155803.h5'))
