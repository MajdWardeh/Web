...
class Network:
    ...
    def __init__(self, config):
        ...
        self.numOfUnitsPerLayer = config['numOfUnitsPerLayer'] # list 
        self.dropRatePerLayer = config['dropRatePerLayer'] # list
        # self.numOf1DConvFiltersPerLayer = config['numOf1DConvFiltersPerLayer'] # list of lists
    ...
    def _createModel(self):
        ...
        twistDataInput = layers.Input(shape=(400, ))

        # twistDataX = layers.Conv1D(filters=32, kernel_size=3, input_shape=(100, 4))(twistDataInput)
        # twistDataX = layers.Conv1D(filters=32, kernel_size=3)(twistDataX)
        # twistDataX = layers.Activation('relu')(twistDataX)
        # twistDataX = layers.MaxPool1D(pool_size=2)(twistDataX)

        # twistDataX = layers.Conv1D(filters=64, kernel_size=3)(twistDataX)
        # twistDataX = layers.Conv1D(filters=64, kernel_size=3)(twistDataX)
        # twistDataX = layers.Activation('relu')(twistDataX)
        # twistDataX = layers.MaxPool1D(pool_size=2)(twistDataX)
        # twistDataX = layers.Flatten()(twistDataX)
...
class Training:
    ...
    def trainModel(self):
        ...
        try:
            ...
            with open(os.path.join(self.saveHistoryDir, 'history_MarkersToBeizer_FC_scratch_withYawAndTwistData400_config{}_{}.pkl'.format(self.configNum, datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))), 'wb') as file_pi:
                pickle.dump(history.history, file_pi) 
        except KeyboardInterrupt:
            print('KeyboardInterrupt, model weights were saved.')
        finally:
            self.model.save_weights(os.path.join(self.model_weights_dir, 'weights_MarkersToBeizer_FC_scratch_withYawAndTwistData400_config{}_{}.h5'.format(self.configNum, datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))))
