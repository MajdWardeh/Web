...
class Unet:
    ...
    def _create_unet(self):
        ...
        outputs = layers.Conv2D(4, (1, 1), activation='linear')(c9)
...
class Training:
    ...
    def trainModel(self):
        try:
            ...
            with open('./trainHistoryDict/history'.format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S")), 'wb') as file_pi:
                pickle.dump(history.history, file_pi) 
        except KeyboardInterrupt:
            print('KeyboardInterrupt, model weights were saved.')
        finally:
            self.model.save_weights('./model_weights/weights_unet_scratch_{}.h5'.format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S")))
    ...
    def testModel(self):
        ...
        for k in range(self.testGen.__len__()):
            ...
            y_hat = y_hat[0]
            # y_hat_gray = np.zeros(shape=(y_hat[0], y_hat[1]), dtype=np.float32)
            x = (x[0] * 128).astype(np.uint8)
            y_hat_gray = (np.sum(y_hat, axis=2) * 255).astype(np.uint8)
            y_hat_gray = y_hat_gray[:, :,  np.newaxis]
            y_hat_rgb = x.copy()
            # print(((y_hat_gray != 0).reshape(480, 640)).shape)
            y_hat_rgb[(y_hat_gray != 0).reshape(480, 640)] = y_hat_gray[(y_hat_gray!=0).reshape(480, 640)]
            cv2.imshow('image', x)
            cv2.imshow('y_hat', y_hat_rgb)
            cv2.imshow('y', y[0])
            cv2.waitKey(0)
